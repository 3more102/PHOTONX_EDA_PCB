...
- Hardened generic Gerber X2 attribute ingestion against the Ucamco 2026.05 grammar: TF/TA/TO standard names are now checked against their attribute domains, reserved dot-prefixed names must be recognized, TD cannot carry values, user-defined names follow the specification grammar, and malformed attributes fail closed (with complete file-geometry suppression in permissive mode) instead of being preserved as harmless metadata.
