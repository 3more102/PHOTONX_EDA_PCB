...
- Isolated mutable parser state per input file when reusing `GerberRS274XParser` or `ExcellonParser`. Each `parse()` call now starts from clean format/unit, aperture/tool, coordinate, routing/region, polarity, and fail-closed geometry state so evidence from one manufacturing file cannot contaminate the next; strict and permissive reuse regressions cover both parsers.
