...
- Hardened Gerber layer-polarity handling: `%LPD*%` remains supported, while `%LPC*%` fails closed so clear subtraction cannot be emitted as positive copper; permissive parsing suppresses the affected file geometry.
- Hardened Gerber LP layer-polarity handling: `%LPD*%` remains supported, while `%LPC*%` now fails closed in strict mode and suppresses file geometry in permissive mode so clear objects cannot be emitted as positive material.
