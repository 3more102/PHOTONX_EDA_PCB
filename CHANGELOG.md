...
- Reused revision-safe packed native spatial-index snapshots across repeated native calls. `SpatialHashIndex` now exposes a monotonic insertion revision; the weak-keyed cache invalidates on mutation, retains no dead indexes, and also reuses the exact Python-style box centers used by radius post-filtering without changing acceptance semantics.
