...
- Added evidence-gated KiCad export for exact straight plated Excellon routed slots. A plated route is emitted as deterministic `PHOTONX:RecoveredPlatedRoute` only when copper-only multilayer evidence proves one uniform pad-stack and one unambiguous exported net; X2-declared stackups additionally require exact coverage of every declared copper layer. Incomplete or contradictory evidence remains an explicit omission, and round-trip verification checks route kind, geometry, net binding, and deterministic identity.
