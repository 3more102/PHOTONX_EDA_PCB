- Added fail-closed KiCad export for exactly representable proven plated via spans. PhotonX now emits a real through-hole KiCad `via` only for a proven F.Cu↔B.Cu plated span where one concentric circular supporting pad of identical annular diameter exists on every declared copper layer and all support resolves to one unambiguous exported net. Via type/position/size/drill/layers/net/UUID are round-trip verified; partial-layer spans, incomplete layer support, annulus mismatch, ambiguous identity, and net conflicts remain explicit omissions instead of guessing blind/buried or microvia manufacturing semantics.\n\n# Changelog
