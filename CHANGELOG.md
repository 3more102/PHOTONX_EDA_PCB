...
- Enforced production parser resource budgets through `ParseLimits`: Gerber and Excellon now hard-limit physical line count/length while reading input, before semantic parsing; Excellon consumes lines as a stream instead of loading the whole drill file first, while Gerber validates physical lines while assembling tokenizer input. Gerber additionally bounds unique aperture D-codes, aperture-macro definitions, and emitted geometry; Excellon bounds unique tool definitions and emitted drill/slot/route objects. Limit violations raise `ParseError` in both strict and permissive modes, with focused low-budget regressions so hostile or accidental oversized inputs fail deterministically instead of growing parser state without bound.
