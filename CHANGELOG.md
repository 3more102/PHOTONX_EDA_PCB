...
- Added exact linear D01 sweeps for valid single-positive general Gerber Code-4 outline macros on material layers. Irregular, concave, and off-center outlines are swept as the union of endpoint polygons and swept edge faces rather than a convex hull, preserving concavities exactly; LM/LR/LS, supported whole-image IR, step-repeat, ordered LPD/LPC composition, deterministic IDs, and `gerber_outline_macro_track` provenance are preserved. Code-4 Edge.Cuts geometry remains fail-closed.
