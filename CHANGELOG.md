...
- Expanded KiCad multilayer export: the board layer table now declares every observed canonical `In1.Cu` through `In30.Cu` layer with KiCad's fixed copper ordinals, and simple hole-free CopperRegion zones can be emitted on those declared inner layers instead of being surface-only.
