...
- Hardened Excellon/XNC end-of-file and tool-table validation. `M30` is now terminal and non-comment trailing data fails closed instead of leaking geometry; duplicate tool definitions and undefined tool selections are rejected; malformed, overflowed, non-finite, zero, and negative tool diameters are normalized into explicit parser diagnostics. Strict parsing raises deterministic `ParseError`s, while permissive parsing suppresses file geometry and preflight reports `INVALID_EXCELLON_*` blockers.
