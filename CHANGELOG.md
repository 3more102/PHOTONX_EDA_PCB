...
- Hardened KiCad PCB export for multilayer reconstructions: the required board layer table now declares observed canonical `In1.Cu` through `In30.Cu` copper layers with their KiCad ordinals, so emitted inner-copper tracks no longer reference undeclared layers. Two-layer `F.Cu`/`B.Cu` behavior and conservative geometry-omission policy remain unchanged.
