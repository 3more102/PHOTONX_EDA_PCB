...
- Wired existing `ParseLimits` line-count and line-length guards into the production Gerber and Excellon parsers. Excellon now streams physical lines instead of loading the complete drill file up front; Gerber validates physical lines while assembling tokenizer input. Resource-limit violations stop parsing in strict and permissive modes with path/line context.
