...
- Hardened KiCad net-reference export: objects with a non-empty unknown `net_id` are no longer silently rewritten as net 0. Recovered pads preserve physical geometry without a net claim, while track segments with unresolved net references are explicitly omitted and reported because KiCad requires a segment net ordinal.
