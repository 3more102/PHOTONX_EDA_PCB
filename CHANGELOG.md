...
- Hardened Excellon tool-identifier lexing to ASCII decimal digits across production and helper parsers. Unicode digit lookalikes no longer alias tool numbers, malformed tool selections receive a dedicated `INVALID_EXCELLON_TOOL_SELECTION` diagnostic, permissive parsing clears file geometry, and preflight reports the condition as a strict blocker.
