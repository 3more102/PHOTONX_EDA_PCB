...
- Added specification-aligned Gerber aperture-macro variable definitions and Code-1 validation hardening. Ordered `$n=expression` assignments now use AD-supplied variables, undefined variables evaluate as zero, numeric variable tokens cannot alias longer names, and redefinition fails closed. Code-1 circle reduction now accepts only the supported four/five-modifier forms and rejects non-finite exposure, diameter, center, or rotation values before geometry creation.
