...
- Hardened Excellon end-of-file handling: M30 now terminates the drill program, strict parsing rejects any later non-empty command, permissive parsing suppresses file geometry with an explicit blocker, and preflight reports the violation instead of allowing trailing drill or route data to leak into reconstruction.
