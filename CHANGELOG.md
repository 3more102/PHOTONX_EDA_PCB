...
- Upgraded the optional native spatial backend to ABI v3 with persistent immutable AABB index handles. Repeated candidate-pair queries on an unchanged `SpatialHashIndex` now reuse the C++ spatial hash, successful inserts advance a revision that invalidates/rebuilds the cached handle on the next native query, generic index-like objects remain temporary-handle safe, and deterministic Python/native parity semantics are preserved.
