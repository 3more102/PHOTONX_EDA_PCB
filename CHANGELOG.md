...
- Added bounded exact Gerber aperture-block reduction for blocks containing one explicit origin-centered D03 flash of an already-supported aperture. Definition is accepted only under dark polarity, identity LM/LR/LS and legacy MI/SF/OF/IR image-transform state, no active step-repeat, and no active G36 region; the resulting block D-code reuses the existing supported flash geometry. Composite, off-origin, draw/region, transformed-at-definition, clear-polarity, nested, malformed, and unterminated blocks remain fail-closed, with permissive parsing suppressing the file image so block bodies cannot leak as ordinary geometry. Parser, preflight, and conformance regressions cover the boundary.
