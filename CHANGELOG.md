...
- Hardened Excellon unit-state handling: once metric or inch units are declared through `METRIC/INCH/M71/M72`, a later switch to the opposite unit system now fails closed. Strict parsing raises an error, permissive parsing suppresses all file geometry with `CONFLICTING_EXCELLON_UNITS`, and preflight reports the conflict as a strict blocker while same-unit redeclarations remain supported.
