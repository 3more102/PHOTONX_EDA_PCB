...
- Added specification-faithful Gerber aperture-macro variable evaluation: calling AD parameters seed `$1..$n`, ordered `$n=expression` statements define new variables, undefined variables evaluate to zero without prefix aliasing, redefinition is rejected, and non-finite values fail closed. Circle primitive validation now accepts exactly the required four parameters plus optional rotation.
