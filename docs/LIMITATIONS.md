...
- generic X2 `TF`/`TA`/`TO`/`TD` commands are grammar-validated; active `TA` aperture attributes are snapshotted when a standard or macro aperture is defined and later attached as immutable provenance to geometry created with that aperture, while current `TO` object-attribute state is snapshotted onto each subsequently created graphical object. `TD` deletion is non-retroactive for already-created apertures/objects, `.AperFunction` is preserved as structured aperture-function evidence, and user-defined names follow the single aperture/object attribute dictionary namespace. `.N` values are preserved as source net-name evidence. Physical nets receive a `.N` label only when the source name is non-reserved, internally consistent, and unique across reconstructed physical groups; empty `.N`, `N/C`, conflicting names, and duplicate names remain explicit unresolved evidence. Standard-name domains, Unicode escapes, reserved characters, attribute names, required `TO.C`/`TO.N` value fields, and attribute commands inside regions fail closed when malformed;
