...
- unsupported legacy Gerber `AS/IP/OF/SF` transforms: strict mode rejects them and permissive mode suppresses file geometry rather than emitting incorrect image semantics; `MI` and `IR` are modeled exactly within their declared specification-defined subsets;
