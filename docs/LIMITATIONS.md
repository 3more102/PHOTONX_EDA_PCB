...
- zero, malformed, non-finite, or duplicate Excellon tool definitions and undefined tool selections: strict mode rejects them and permissive mode clears/suppresses file geometry instead of producing ambiguous or invalid drills, slots, or routes;\n- non-comment data after Excellon/XNC `M30`: strict mode rejects the file and permissive mode reports `INVALID_EXCELLON_DATA_AFTER_M30` while suppressing file geometry; comments and blank lines after `M30` remain non-geometric;
