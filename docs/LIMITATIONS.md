...
- Gerber aperture blocks containing exactly one explicit origin-centered D03 flash of an already-supported aperture are reduced exactly when the block is defined under dark polarity, identity LM/LR/LS and legacy MI/SF/OF/IR image-transform state, no active step-repeat, and no active G36 region; the resulting D-code reuses the normal supported flash path, while composite/off-origin/draw/region/transformed/clear/nested blocks remain fail-closed;
...
- ambiguous/invalid G74 center cases and complex aperture macros; mixed-axis or otherwise invalid cut-ins, fully-coincident cut-ins that resolve to disjoint filled areas, LPC files containing outline material geometry, plus Edge.Cuts regions and unsupported composite/off-origin/transformed aperture blocks, remain fail-closed rather than being approximated or leaked as ordinary geometry;
