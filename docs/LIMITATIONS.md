...
- generic X2 `TF`/`TA`/`TO`/`TD` commands are grammar-validated against the Ucamco 2026.05 name/domain rules before being retained as diagnostics; malformed standard-domain usage, unknown reserved dot-prefixed names, invalid names, and value-bearing `TD` commands fail closed rather than being treated as harmless metadata;
