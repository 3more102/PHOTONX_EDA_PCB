...
- dark Gerber G36/G37 region statements with multiple explicitly closed contours, linear segments, G75 G02/G03 circular boundaries, and one bounded simple cut-in hole per contour when its bridge is a fully-coincident opposite horizontal/vertical linear pair. Holes are first-class CopperRegion geometry and therefore participate in connectivity, DRC, validation, supported whole-image transforms, step-repeat expansion, roundtrip canonicalization, provenance, and explicit KiCad-export omission reporting. Circular boundaries are deterministically tessellated to a 0.005 mm maximum output chord-error target;
...
- ambiguous/invalid G74 center cases and complex aperture macros; G74 circular boundaries inside regions, multiple cut-ins in one contour, fully-coincident cut-ins that resolve to disjoint filled areas, clear-polarity regions, Edge.Cuts regions, and aperture blocks remain fail-closed rather than being approximated or leaked as ordinary tracks/flashes;
