...
- single positive origin-centered circle aperture macros, including parameterized diameters, reduced exactly to circular apertures; centered-circle rotation is geometry-invariant;\n- aperture-macro local variable definitions are evaluated in source order with AD-supplied `$1..$n`, specification-defined zero values for undefined variables, and fail-closed rejection of variable redefinition;
