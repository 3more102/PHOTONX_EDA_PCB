...
- single positive Code-4 outline aperture macros are supported as exact D03 flashes on material layers when they declare an integer 3–5000 vertex count, explicitly repeat the start point as the final point, contain finite coordinates/rotation, and form one valid non-self-intersecting non-zero-area contour; primitive rotation composes with LM/LR/LS, supported whole-image IR, step-repeat, provenance, and LPD/LPC; Code-4 D01 sweeps and Edge.Cuts flashes remain fail-closed;
...
- ambiguous/invalid G74 center cases and unsupported complex aperture macros (including Code-4 D01 sweeps, invalid/self-intersecting Code-4 contours, exposure-off or multi-primitive macros); mixed-axis or otherwise invalid cut-ins, fully-coincident cut-ins that resolve to disjoint filled areas, LPC files containing outline material geometry, plus Edge.Cuts regions and aperture blocks, remain fail-closed rather than being approximated or leaked as ordinary geometry;
