...
- Gerber aperture blocks containing exactly one explicit origin-centered D03 flash of an already-supported aperture are reduced exactly when the block is defined under dark polarity, identity LM/LR/LS state, and no active step-repeat; the resulting D-code reuses the normal supported flash path, while composite/off-origin/draw/region/transformed/clear/nested blocks remain fail-closed;
