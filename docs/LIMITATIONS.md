...
- Gerber G36/G37 region statements with multiple explicitly closed contours, linear segments, bounded G74 single-quadrant and G75 multi-quadrant G02/G03 circular boundaries, and multiple cut-in holes per contour when every bridge is a fully-coincident opposite horizontal/vertical linear pair and all cut-ins in that contour share one axis direction. Files containing supported regions and rectangular D03 flashes may switch between LPD/LPC; ordered clear subtraction is materialized as first-class CopperRegion shells/holes that participate in connectivity, DRC, validation, supported whole-image transforms, step-repeat expansion, roundtrip canonicalization, provenance, and explicit KiCad-export omission reporting. Circular boundaries are deterministically tessellated to a 0.005 mm maximum output chord-error target;
...
- ambiguous/invalid G74 center cases and complex aperture macros; mixed-axis or otherwise invalid cut-ins, fully-coincident cut-ins that resolve to disjoint filled areas, LPC files containing tracks, outline material geometry, or circular/obround flashes, plus Edge.Cuts regions and aperture blocks, remain fail-closed rather than being approximated or leaked as ordinary geometry;
...
- Gerber `%LPC*%` clear layer polarity outside the bounded polygon-exact subset: supported G36/G37 regions and rectangular D03 flashes may compose together, while tracks, outline geometry, and circular/obround flashes cause strict rejection and complete permissive-image suppression rather than partial composition;
