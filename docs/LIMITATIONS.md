...
- single positive Code-4 outline macros use exact R/P reduction when the contour is a centered rectangle or centered regular polygon, while other valid explicitly closed simple contours with 3–5000 vertices are retained as exact linear CopperRegion geometry for D03 flashes and exact translational D01 polygon sweeps on material layers. Irregular, concave, and off-center geometry preserves primitive rotation plus LM/LR/LS, supported whole-image IR, step-repeat, provenance, and LPD/LPC. Zero-length, self-touching, self-intersecting, open, off-exposure, or zero-area contours remain fail-closed, as does Code-4 Edge.Cuts geometry;
