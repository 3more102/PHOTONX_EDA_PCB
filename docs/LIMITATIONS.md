...
- generic X2 `TF`/`TA`/`TO`/`TD` commands are grammar-validated before being retained as diagnostics; standard-name domains, Unicode escapes, reserved characters, attribute names, and TD/TO.C shape rules fail closed when malformed;
