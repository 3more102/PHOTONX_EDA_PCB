...
- ambiguous/invalid G74 center cases and complex aperture macros; mixed-axis or otherwise invalid cut-ins, fully-coincident cut-ins that resolve to disjoint filled areas, LPC files containing tracks/flashes/outline material geometry, Edge.Cuts regions, and aperture blocks remain fail-closed rather than being approximated or leaked as ordinary geometry;
...
- Gerber `%LPC*%` clear layer polarity outside the bounded region-only subset: if a file also contains tracks, flashes, or outline material geometry, strict mode rejects it and permissive mode suppresses the complete file image rather than composing only part of the semantics;
