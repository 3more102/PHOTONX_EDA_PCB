...
## Stage 3: batch AABB queries

The native backend can evaluate a batch of AABB intersection queries against one spatial hash build. The Python-facing `aabb_queries()` API preserves `SpatialHashIndex.query()` semantics: inclusive boundary intersection, deterministic string-ID ordering, and duplicate-free results even when one box occupies several grid cells.

`backend="auto"` falls back to the Python reference implementation when the native range guard rejects an input. `backend="native"` exposes that rejection for verification and benchmarking.
...
1. persistent native index handles to avoid rebuilding grids across independent batch calls;
2. benchmark-gated crossover thresholds and performance regression tooling;
3. broader cross-platform packaging of the optional native library;
