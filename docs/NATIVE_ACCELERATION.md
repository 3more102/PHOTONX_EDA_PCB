...
Repeated native operations on the same `SpatialHashIndex` reuse a packed `ctypes` AABB snapshot. The cache is keyed by index identity plus a monotonic insertion revision, so any index mutation invalidates the snapshot before the next native call. Weak keys ensure this optimization does not keep discarded indexes alive.
