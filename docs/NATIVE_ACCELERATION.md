...
Repeated radius-query batches now reuse an opaque C++ point-index handle instead of rebuilding the native center-point hash on every ctypes call. `SpatialHashIndex` carries a monotonic mutation revision; the Python bridge caches the native handle weakly and rebuilds it automatically after an insertion. Cache reset also destroys native handles before the shared-library probe is cleared.
