...
Deprecated `OF` applies an absolute translation in the active MO units to the complete image. The transformation order follows the legacy Gerber rule independently of command appearance: MI is applied first, then SF, then OF translation, then IR rotation. Legacy once-only whole-image/header commands, including positive image polarity IPPOS and the supported AS/IN/MI/SF/OF/IR subset, are accepted only before the first coordinate statement. A D02 move therefore closes the header even though it emits no physical object; late or duplicate header state is rejected in strict parsing and reported as a strict preflight blocker. IPNEG remains fail-closed because negative whole-image inversion requires an explicit bounded image-plane model.
