...
- **Code 4 outline:** exposure on, integer vertex count from 3 through 5000, an explicitly closed finite linear contour, valid non-self-intersecting topology, non-zero area, and finite primitive rotation. The contour may be offset from the macro origin. It is retained as exact polygon geometry for `D03` flashes on material layers; LM/LR/LS, supported whole-image IR, step-repeat, provenance, and ordered LPD/LPC composition are preserved. Arbitrary `D01` sweeps and `Edge.Cuts` flashes remain fail-closed.
...
Multiple primitives, subtraction/exposure-off geometry, non-centered rectangles/vector lines/polygons, zero-size primitives, unsupported polygon forms, Code-4 outline D01 sweeps, invalid/self-intersecting outline contours, thermals, moirés, and aperture blocks remain outside this production path unless a later implementation can preserve their geometry exactly.
