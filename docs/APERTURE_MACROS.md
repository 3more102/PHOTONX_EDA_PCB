...
Multiple primitives, subtraction/exposure-off geometry, non-centered rectangles/vector lines/Code-5 polygons, zero-size primitives, invalid Code-4 contours, general Code-4 D01 sweeps, Edge.Cuts Code-4 flashes, thermals, and moirés remain outside the supported macro path. Separately, aperture blocks have a bounded exact reduction only when they contain one explicit origin-centered D03 flash of an already-supported aperture; composite/off-origin/draw/region blocks and definitions under non-identity aperture/image transforms remain fail-closed.
