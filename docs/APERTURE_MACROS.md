...
- **Code 4 outline:** exposure on, exactly four vertices plus the repeated closing point, finite coordinates/rotation, explicit closure, and geometry that is provably an origin-centered rectangle. The first edge orientation plus primitive rotation are retained as the intrinsic rotation of an exact rectangular aperture.
