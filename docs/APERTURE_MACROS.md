...
Macro modifiers may be parameterized and are evaluated before these constraints are checked. Macro-local variable definitions such as `$4=$1x1.25-$3` are evaluated in source order. Variables supplied by the calling `AD` command cannot be redefined, previously defined macro variables cannot be redefined, and undefined variables evaluate as zero, matching the current Gerber specification. Variable substitution is token-aware, so `$1` never aliases `$10`. Variable-definition statements do not count as image primitives when applying the single-supported-primitive production reduction rule. Active Gerber units are applied when the primitive is reduced.
