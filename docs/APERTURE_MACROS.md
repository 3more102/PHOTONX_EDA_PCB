...
General valid Code-4 outlines also support exact linear D01 translational sweeps on material layers. The sweep is constructed from the two endpoint outline polygons plus the swept face of every contour edge, so concave geometry is preserved instead of being filled by a convex hull. Multiple primitives, subtraction/exposure-off geometry, non-centered rectangles/vector lines/Code-5 polygons, zero-size primitives, invalid Code-4 contours, Code-4 Edge.Cuts geometry, thermals, moirés, and aperture blocks remain outside the supported production path unless a later implementation can preserve their geometry safely.
