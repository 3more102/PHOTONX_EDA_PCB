...
Macro modifiers may be parameterized and are evaluated before these constraints are checked. Ordered local definitions such as `$4=$1x1.25-$3` are evaluated in source order. AD modifiers seed `$1..$n`; undefined variables evaluate as zero; `$0`, AD-variable redefinition, and repeated local definitions are invalid and fail closed. Variable names are tokenized exactly, so `$1` never aliases `$10`. Active Gerber units are applied when the primitive is reduced.
