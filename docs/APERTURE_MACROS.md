...
- **Code 20 vector line:** exposure on, positive width, zero rotation, origin-centered midpoint, and a non-zero axis-aligned segment. The result is an exact standard rectangular aperture.
...
Multiple primitives, subtraction/exposure-off geometry, non-centered or rotated vector lines, diagonal vector lines, offsets, rotations, zero-size primitives, outlines, polygons, thermals, moirés, and aperture blocks remain outside this production reduction path unless a later implementation can preserve their geometry exactly.
