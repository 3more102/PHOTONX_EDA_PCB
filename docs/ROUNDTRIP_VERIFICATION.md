...
- exact singleton board-section cardinality for `general`, `paper`, `layers`, and `setup`, plus exact generated paper value `A4`; duplicate/missing sections or paper-size drift fail round-trip verification instead of relying on first-match parsing of an ambiguous board document;
