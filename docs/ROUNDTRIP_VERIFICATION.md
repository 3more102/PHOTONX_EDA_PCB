...
- a canonical `electrical_connectivity_partition` across emitted track segments, exactly proven vias, recovered pads, copper regions, and plated slots. Each conductive artifact is keyed by its preserved source identity or deterministic UUID and grouped by exported net, so cross-family net splits, merges, duplicate identities, or rebindings are fail-visible as one topology invariant independent of geometry;
