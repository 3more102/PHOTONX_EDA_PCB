...
- fail-closed rejection of any direct board/footprint item carrying F/B.Mask or F/B.Paste as its singular layer; PhotonX mask/paste output is defined only by audited pad layer sets and fabrication margins, so graphics, images, dimensions, or future direct-layer artwork cannot bypass the audit;
