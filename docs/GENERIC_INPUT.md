...
- Gerber aperture macros may use ordered local `$n=expression` definitions; calling AD parameters seed `$1..$n`, undefined variables evaluate as zero, variable references are token-matched exactly, and redefinition is rejected as invalid;
