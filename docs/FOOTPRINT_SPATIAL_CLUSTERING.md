...
When enabled, each proximity-connected group is deterministically partitioned so every pad admitted to a guarded cluster is within the configured center-to-center span of every pad already in that cluster. This prevents long transitive pad chains from silently merging adjacent component candidates.
