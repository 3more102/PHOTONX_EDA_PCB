...
def test_macro_validation_accepts_variable_definition_statements():
...
def test_undefined_longer_variable_does_not_alias_defined_prefix():
    statements = parse_macro_body("1,1,$10,0,0")

    evaluated = evaluate_macro(statements, {"1": 0.8})

    assert evaluated[0]["values"] == pytest.approx([1.0, 0.0, 0.0, 0.0])
...
def test_production_parser_rejects_macro_expression_overflow(tmp_path: Path):
    huge_integer = "9" * 400
    path = _write(
        tmp_path,
        "%FSLAX24Y24*%\n"
