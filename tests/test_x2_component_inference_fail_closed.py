...
def test_source_refdes_groups_unique_pins_before_distance_pairing():
    board = BoardModel(
        pads=[
    ...
    geometry = [
        c
        for c in components
        if c.kind == "two_pad_component_candidate"
    ]
...
def test_step_repeat_instances_keep_reused_refdes_separate():
    board = BoardModel(
        pads=[
            _pad(
                "A1",
                ...
                pin="1",
                step_repeat=(
                    "instance=(1,1)/(2,1); source_offset_mm=(0,0)"
                ),
            ),
            _pad(
                "A2",
            ...
            _pad(
                ...
                pin="1",
                step_repeat=(
                    "instance=(2,1)/(2,1); source_offset_mm=(20,0)"
                ),
            ),
            _pad(
                "B2",
    ...
    source = [
        c
        for c in infer_component_hypotheses(
            board,
            max_pair_distance_mm=2.0,
            backend="python",
        )
        if c.kind == "gerber_x2_component"
    ]
...
def test_expanded_panel_duplicate_pins_fail_closed_without_step_repeat():
    board = BoardModel(
        pads=[
    ...
    assert not any(
        c.kind == "gerber_x2_component"
        for c in components
    )
    ambiguous = [
        c
        for c in components
        if c.kind == "gerber_x2_component_ambiguous"
    ]
    assert len(ambiguous) == 4
    assert {tuple(c.pad_ids) for c in ambiguous} == {
        ("A1",),
        ("A2",),
        ("B1",),
        ("B2",),
    }
    assert all(c.reference == "U1" for c in ambiguous)
    assert all(c.confidence == 0.0 for c in ambiguous)
    assert not any(
        len(c.pad_ids) > 1 and c.reference == "U1"
        for c in components
    )


def test_conflicting_refdes_is_not_reassigned_by_geometry():
    ...
    assert ambiguous[0].pad_ids == ["P1"]
    assert ambiguous[0].reference is None
    assert ambiguous[0].confidence == 0.0

    unresolved = [
        c
        for c in components
        if c.kind == "unresolved_pad"
    ]
...
def test_non_proven_refdes_is_not_promoted_or_geometrically_reassigned():
    board = BoardModel(
        pads=[
            _pad(
                "P1",
