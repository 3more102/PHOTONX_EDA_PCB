...
def test_same_name_ta_and_to_share_one_current_dictionary_entry(tmp_path: Path):
    path = _write(
        tmp_path,
        "%FSLAX24Y24*%\n"
        "%MOMM*%\n"
        "%TACustom,aperture-value*%\n"
        "%TOCustom,object-value*%\n"
        "%ADD10C,1.000*%\n"
        "D10*\n"
        "X010000Y010000D03*\n"
        "M02*\n",
    ...
    assert _evidence(pad, "gerber_x2_aperture_attribute") == []
    assert any(
        "name=Custom" in detail and "object-value" in detail
        for detail in _evidence(pad, "gerber_x2_object_attribute")
    )
