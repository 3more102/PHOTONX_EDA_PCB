...
def test_macro_variable_definition_is_evaluated_in_source_order():
    statements = parse_macro_body(
        "$3=$1x1.25*$4=$3+$2*1,1,$4,0,0"
    )
...
def test_undefined_macro_variables_default_to_zero_without_prefix_aliasing():
    ...
    statements = parse_macro_body("1,1,$10+$1,0,0")
    evaluated = evaluate_macro(statements, {"1": 0.8})

    assert evaluated[0]["values"] == pytest.approx([1.0, 0.8, 0.0, 0.0])
...
def test_nonfinite_macro_values_fail_closed_at_evaluation():
    statements = parse_macro_body("$2=1e309*1,1,$2,0,0")
...
def test_production_parser_uses_zero_for_undefined_longer_variable(tmp_path: Path):
    path = _write(
        tmp_path,
        "%FSLAX24Y24*%\n"
        "%MOMM*%\n"
        "%AMDEFAULT*1,1,$10+$1,0,0*%\n"
        "%ADD10DEFAULT,0.8*%\n"
        "D10*\n"
        "X000000Y000000D03*\n"
        "M02*\n",
...
def test_production_parser_rejects_nonfinite_macro_values(
    tmp_path: Path,
    macro_body: str,
):
    path = _write(
