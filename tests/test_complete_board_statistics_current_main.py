...
from photonx_eda_pcb.board_statistics import (
    BoardStats,
    compute_board_stats,
    validate_stats,
)
...
def test_extended_statistics_validation_covers_counts_lengths_and_dimensions():
    stats = compute_board_stats(_board_with_extended_physical_evidence())
    invalid = replace(
        stats,
        routes=-1,
    ...
    assert "STATS_NEGATIVE_BOARD_HEIGHT" in issues
...
def test_legacy_board_stats_constructor_keeps_extended_defaults():
    stats = BoardStats(1, 2, 3, 4, 5, 6, 7.0)

    assert stats.slots == 0
    assert stats.routes == 0
    assert stats.regions == 0
    assert stats.diagnostics == 0
    assert stats.route_length_mm == 0.0
    assert stats.outline_length_mm == 0.0
