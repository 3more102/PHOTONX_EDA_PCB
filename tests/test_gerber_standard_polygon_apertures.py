...
from photonx_eda_pcb.gerber_image import polygonize_regular_polygon_flash
...
def test_polygon_d01_draw_remains_explicitly_fail_closed(tmp_path: Path):
    path = _write(
        tmp_path,
        "%ADD10P,1X6*%",
        "X000000Y000000D02*\n"
    ...
    with pytest.raises(UnsupportedFeatureError, match="P polygon aperture"):
        GerberRS274XParser("F.Cu", strict=True).parse(path)

    report = preflight(path)
    assert not report.ready_for_strict_reconstruction
    assert any(
        "UNSUPPORTED_GERBER_POLYGON_DRAW" in blocker
        for blocker in report.strict_blockers
    )


def test_permissive_polygon_draw_advances_current_and_recovers(tmp_path: Path):
    ...
    assert any(
        diagnostic.code == "UNSUPPORTED_GERBER_POLYGON_DRAW"
