...
def test_capability_catalog_is_structurally_valid():
    ...
    assert index["Gerber step-and-repeat"].status == "implemented"
...
def test_limitations_document_matches_current_parser_boundary():
    text = Path("docs/LIMITATIONS.md").read_text(encoding="utf-8")

    assert "Gerber step-and-repeat expansion" in text
