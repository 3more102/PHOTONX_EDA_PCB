...
class RecordingCanvas:
    ...
    _draw_polygon = BoardCanvas._draw_polygon
    _net_dash = staticmethod(BoardCanvas._net_dash)
...
def test_canvas_renders_all_board_manufacturing_evidence_without_tk_root():
    region = CopperRegion(
        "REGION",
        (
            Point(0, 0),
    ...
    tagged = [
        (kind, kwargs.get("tags"), kwargs.get("dash"))
        for kind, _args, kwargs in canvas.calls
        if "tags" in kwargs
    ]

    assert ("polygon", ("REGION", "region"), (2, 3)) in tagged
    assert ("polygon", ("REGION", "region-hole"), (2, 3)) in tagged
    assert ("line", ("TRACK", "track"), (2, 3)) in tagged
    assert ("oval", ("PAD", "pad"), (2, 3)) in tagged
    ...
    assert sum(
        kind == "line" and tags == ("ROUTE", "route")
        for kind, tags, _ in tagged
    ) == 2
