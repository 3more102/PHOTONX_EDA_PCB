...
|---|---|---|
| Gerber linear draws / flashes | **Partial** | Solid C/R/O flashes are supported. Standard C/R/O D03 flashes with one centered round-hole modifier are materialized as transparent-hole CopperRegion polygons. Circular D01 draws remain centerline tracks; rectangular D01 sweeps are exact CopperRegion polygons and obround D01 sweeps use deterministic 0.005 mm chord-error-bounded polygonization. Arbitrary finite LR is supported for R/O linear sweeps and flashes; holed D01/G02/G03 draws remain fail-closed |
...
| Gerber layer polarity | **Partial** | `LPD` is supported; `LPC` composes supported G36/G37 regions, solid or round-holed C/R/O D03 flashes, linear C/R/O D01 aperture sweeps, and circular-aperture G02/G03 tracks in source order. Aperture holes are transparent within the flash operation rather than separate clear operations. Regions, R flashes, and R linear sweeps are polygon-exact; C/O curved boundaries use deterministic inscribed chords with a 0.005 mm maximum chord-error target. Tessellated circular-aperture arc tracks retain a conservative <=0.010 mm combined boundary-error budget; outline geometry remains fail-closed |
...
| Gerber C/R/O apertures | **Implemented subset** | Positive-size C/R/O apertures are modeled, and the specification-defined zero-diameter C aperture is accepted as a legal no-image object. A single centered round-hole modifier is supported for D03 flashes on material layers as transparent CopperRegion geometry; zero-size/non-fitting holes and holed D01/G02/G03 draws remain fail-closed |
...
| Gerber simple aperture macros | **Partial** | Origin-centered circles plus exact centered Code-20/Code-2 vector-line, Code-21 center-line, and deprecated Code-22 lower-left rectangles are reduced to C/R apertures; rectangle rotations are supported only in 90-degree steps |
| Gerber regions / complex macros | **Partial** | G36/G37 statements support multiple closed linear/G74/G75 contours plus bounded cut-in holes; LPD/LPC streams containing supported regions, solid C/R/O D03 flashes, and linear circular-aperture D01 tracks are composed in order and materialized as explicit CopperRegion shells/holes with provenance. C/O flash and track-cap polygonization is explicitly evidenced and bounded to 0.005 mm chord error. Disjoint/mixed-axis/invalid cut-ins, LPC with outlines, Edge.Cuts regions, aperture blocks, and complex macros remain fail-closed |
...
- Gerber `%LPC*%` clear layer polarity uses ordered polygon composition for supported G36/G37 regions, solid C/R/O D03 flashes, linear C/R/O D01 aperture sweeps, and circular-aperture G02/G03 tracks. Regions, R flashes, and R linear sweeps are exact; C/O curved boundaries use deterministic inscribed-chord polygonization with explicit `gerber_flash_polygonization` / `gerber_track_polygonization` evidence and a 0.005 mm maximum chord-error target. Tessellated circular-aperture arc tracks retain the documented combined error bound; outlines remain fail-closed.
...
- Deprecated `MI`, `SF`, `OF`, and orthogonal `IR` image transforms are modeled in the specification-defined MI → SF → OF → IR order. Legacy AS/IN/MI/SF/OF/IR header state must appear before the first coordinate statement (including D02 moves), and duplicate/late header diagnostics are strict preflight blockers. SF scales coordinate data only: apertures and step-repeat distances remain unchanged; uniform SF supports circular arcs while anisotropic SF arcs fail closed rather than becoming implicit ellipses. Both `ASAXBY` and `ASAYBX` are accepted as output-device-only no-ops. Legacy `IN/LN` names are retained as diagnostics, `G55/M01` are harmless no-ops, and `M00` terminates parsing like `M02`. Unsupported `IP` semantics remain fail-closed.
