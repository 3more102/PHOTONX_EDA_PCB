...
- Deprecated `MI` mirroring is supported exactly for coordinate data; apertures and step-repeat distances are intentionally not mirrored. Deprecated `OF` translation is supported exactly in active MO units, and `IR` rotation is supported exactly for 0/90/180/270 degrees. The transform order is MI → SF → OF → IR regardless of command appearance. Identity legacy `SF` is accepted; non-identity `SF` and unsupported `AS/IP` transforms remain fail-closed.
