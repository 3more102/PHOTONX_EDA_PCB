...
- Excellon tool diameters must be finite and positive; zero, malformed, or non-finite diameters are rejected in strict mode and suppress permissive file geometry instead of creating invalid physical features.\n- XNC/Excellon end-of-file and tool-table semantics are fail-closed: `M30` is terminal, non-comment data after it is invalid, duplicate tool definitions are rejected, and tool selections must reference a previously defined tool.
