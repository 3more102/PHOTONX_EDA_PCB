...
- Deprecated `MI` mirroring is supported exactly for coordinate data and composes with incremental coordinates, arcs, outlines, and IR; apertures and step-repeat distances are intentionally not mirrored. Deprecated whole-image `IR` rotation is supported exactly for 0/90/180/270 degrees. Identity legacy `SF` is accepted; non-identity `SF` and unsupported `AS/IP/OF` transforms remain fail-closed with permissive geometry suppression.
