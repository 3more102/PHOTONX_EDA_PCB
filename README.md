...
| Gerber regions / complex macros | **Partial** | Dark single-contour G36/G37 regions support linear segments and G75 circular boundaries with bounded tessellation, provenance, supported image transforms, step-repeat, connectivity and validation; G74 region arcs, multi-contour/hole, clear-polarity, Edge.Cuts regions, aperture blocks, and complex macros remain fail-closed |
...
- Single positive origin-centered circle macros plus exact centered Code-20 vector-line (including deprecated Code-2 alias), Code-21 center-line, and deprecated Code-22 lower-left rectangle macros are supported as exact C/R apertures. Rectangle macro rotations are accepted only in 90-degree steps; non-centered, diagonal, non-orthogonal, subtractive, and other complex macro geometry remains fail-closed. Legacy G74 single-quadrant arcs are supported only for unsigned I/J distances when one center candidate is unambiguous and the sweep is at most 90°. Dark single-contour regions with linear/G75 circular boundaries are supported; G74 region arcs, multi-contour/hole regions, clear-polarity regions, Edge.Cuts regions, and aperture blocks remain fail-closed.
