...
| Gerber regions / complex macros | **Partial** | Dark single-contour linear G36/G37 regions are reconstructed as exact copper polygons with provenance, supported image transforms, step-repeat, connectivity and validation; arc-bounded, multi-contour/hole, clear-polarity, Edge.Cuts regions, aperture blocks, and complex macros remain fail-closed |
...
- Single positive origin-centered circle macros plus exact centered Code-20 vector-line (including deprecated Code-2 alias), Code-21 center-line, and deprecated Code-22 lower-left rectangle macros are supported as exact C/R apertures. Rectangle macro rotations are accepted only in 90-degree steps; non-centered, diagonal, non-orthogonal, subtractive, and other complex macro geometry remains fail-closed. Legacy G74 single-quadrant arcs are supported only for unsigned I/J distances when one center candidate is unambiguous and the sweep is at most 90°. Gerber regions and aperture blocks remain outside the declared production high-level parser and suppress file geometry in permissive mode.
