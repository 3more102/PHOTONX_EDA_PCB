...
- Deprecated `MI` mirroring, `OF` translation, and orthogonal `IR` rotation are modeled with the legacy MI → SF → OF → IR order. Both `ASAXBY` and `ASAYBX` are accepted as output-device-only no-ops for CAD-to-CAM geometry. Legacy `IN/LN` names are retained as diagnostics, `G55/M01` are harmless no-ops, and `M00` terminates parsing like `M02`. Non-identity `SF` and unsupported `IP` semantics remain fail-closed.
