...
| Gerber layer polarity | **Partial** | `LPD` is supported; `LPC` composes supported G36/G37 regions, solid C/R/O D03 flashes, and linear circular-aperture D01 tracks in source order. Regions/R flashes are polygon-exact; C/O flash boundaries and track end-caps use deterministic inscribed chords with a 0.005 mm maximum chord-error target. G02/G03 arc tracks are supported through bounded centerline + capsule tessellation with a conservative <=0.010 mm combined boundary-error budget; outline geometry remains fail-closed |
