...
| Gerber layer polarity | **Partial** | `LPD` is supported; `LPC` is exact for region-only G36/G37 files via ordered polygon subtraction/materialization. Files mixing LPC with tracks, flashes, or outline geometry remain fail-closed |
...
| Gerber regions / complex macros | **Partial** | G36/G37 statements support multiple closed linear/G74/G75 contours plus bounded cut-in holes; region-only LPD/LPC streams are composed in order and materialized as explicit CopperRegion shells/holes with provenance. Disjoint/mixed-axis/invalid cut-ins, LPC mixed with tracks/flashes/outlines, Edge.Cuts regions, aperture blocks, and complex macros remain fail-closed |
