...
| Gerber layer polarity | **Partial** | `LPD` is supported; `LPC` is exact for supported G36/G37 regions plus rectangular D03 flashes via ordered polygon subtraction/materialization. Tracks, outline geometry, and circular/obround flashes remain fail-closed |
...
| Gerber regions / complex macros | **Partial** | G36/G37 statements support multiple closed linear/G74/G75 contours plus bounded cut-in holes; LPD/LPC streams containing supported regions and rectangular D03 flashes are composed in order and materialized as explicit CopperRegion shells/holes with provenance. Disjoint/mixed-axis/invalid cut-ins, LPC with tracks/outlines/non-rectangular flashes, Edge.Cuts regions, aperture blocks, and complex macros remain fail-closed |
...
- Gerber `%LPC*%` clear layer polarity uses ordered polygon composition for the bounded exact subset of supported G36/G37 regions plus rectangular D03 flashes. Tracks, outlines, and circular/obround flashes remain fail-closed rather than being flattened through an approximation boundary.
