...
| Gerber regions / complex macros | **Partial** | Dark G36/G37 statements support multiple closed linear/G74/G75 contours plus multiple cut-in holes per contour when every bridge is an opposite fully-coincident linear pair and all bridges share one axis direction; holes are explicit geometry and retain provenance, transforms, step-repeat, connectivity and validation. Disjoint/mixed-axis/invalid cut-ins, clear-polarity, Edge.Cuts regions, aperture blocks, and complex macros remain fail-closed |
...
- Single positive origin-centered circle macros plus exact centered Code-20 vector-line (including deprecated Code-2 alias), Code-21 center-line, and deprecated Code-22 lower-left rectangle macros are supported as exact C/R apertures. Rectangle macro rotations are accepted only in 90-degree steps; non-centered, diagonal, non-orthogonal, subtractive, and other complex macro geometry remains fail-closed. Legacy G74 single-quadrant arcs are supported only for unsigned I/J distances when one center candidate is unambiguous and the sweep is at most 90°. Dark multi-contour regions with linear/G74/G75 circular boundaries are supported; each contour must be explicitly closed and is filled individually with union semantics. Multiple cut-ins in one contour are reconstructed as explicit holes when every bridge is a fully-coincident opposite horizontal/vertical linear pair and all cut-ins in that contour share the same direction, matching the Gerber rule. G74/G75 circular inner boundaries are supported within their bounded arc rules. Disjoint fully-coincident cut-in topology, mixed-axis/invalid cut-ins, clear-polarity regions, Edge.Cuts regions, and aperture blocks remain fail-closed.
