...
- Excellon tool diameters must be finite and positive; malformed, non-finite, duplicate, or zero-diameter tool definitions fail closed, and tool selections must reference a previously defined tool.\n- XNC/Excellon `M30` is terminal: non-comment trailing data fails closed instead of leaking geometry; comments and blank lines after `M30` remain non-geometric.
