...
| Gerber regions / complex macros | **Partial** | Dark G36/G37 statements support multiple closed linear/G75 contours plus multiple cut-in holes per contour when every bridge is an opposite fully-coincident linear pair and all bridges share one axis direction; holes are explicit geometry and retain provenance, transforms, step-repeat, connectivity and validation. Disjoint/mixed-axis/invalid cut-ins, G74 region arcs, clear-polarity, Edge.Cuts regions, aperture blocks, and complex macros remain fail-closed |
