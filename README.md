...
- Deprecated whole-image `IR` rotation is supported exactly for 0/90/180/270 degrees and composes with flashes, linear draws, tessellated arcs, outlines, incremental coordinates, and step-repeat instances. Identity legacy `SF` is accepted; non-identity `SF` and non-default `AS/IP/MI/OF` transforms remain fail-closed with permissive geometry suppression.
