...
- Deprecated `MI`, `SF`, `OF`, and orthogonal `IR` image transforms are modeled in the specification-defined MI → SF → OF → IR order. SF scales coordinate data only: apertures and step-repeat distances remain unchanged; uniform SF supports circular arcs while anisotropic SF arcs fail closed rather than becoming implicit ellipses. Both `ASAXBY` and `ASAYBX` are accepted as output-device-only no-ops. Legacy `IN/LN` names are retained as diagnostics, `G55/M01` are harmless no-ops, and `M00` terminates parsing like `M02`. Unsupported `IP` semantics remain fail-closed.
