...
|---|---|---|
| Gerber linear draws / flashes | **Partial** | Solid C/R/O flashes are supported. Circular D01 draws remain centerline tracks; rectangular D01 sweeps are exact CopperRegion polygons and obround D01 sweeps use deterministic 0.005 mm chord-error-bounded polygonization. Arbitrary finite LR is supported for R/O linear sweeps; holed apertures and non-orthogonal R/O flashes fail closed |
...
| Gerber layer polarity | **Partial** | `LPD` is supported; `LPC` composes supported G36/G37 regions, solid C/R/O D03 flashes, linear C/R/O D01 aperture sweeps, and circular-aperture G02/G03 tracks in source order. Regions, R flashes, and R linear sweeps are polygon-exact; C/O curved boundaries use deterministic inscribed chords with a 0.005 mm maximum chord-error target. Tessellated circular-aperture arc tracks retain a conservative <=0.010 mm combined boundary-error budget; outline geometry remains fail-closed |
| Gerber aperture transforms | **Partial** | Modal LM/LR/LS applies to the original aperture at object creation. Circles accept arbitrary LR; R/O linear D01 sweeps accept arbitrary finite LR, while R/O D03 flashes remain limited to 90-degree-step LR because PadCandidate has no orientation field |
...
- Modern `LM/LR/LS` aperture transforms are modal and apply to the original aperture at object creation. PHOTONX supports the exact representable subset: LM for centered symmetric C/R/O geometry, positive LS scaling, arbitrary LR for circles, and 90-degree-step LR for rectangle/obround flashes. Non-orthogonal R/O flashes fail closed.
