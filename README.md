...
| Gerber layer polarity | **Partial** | `LPD` is supported; `LPC` composes supported G36/G37 regions plus solid C/R/O D03 flashes in source order. Regions/R flashes are polygon-exact; C/O flash boundaries use deterministic inscribed chords with a 0.005 mm maximum chord-error target. Tracks and outline geometry remain fail-closed |
...
| Gerber regions / complex macros | **Partial** | G36/G37 statements support multiple closed linear/G74/G75 contours plus bounded cut-in holes; LPD/LPC streams containing supported regions and solid C/R/O D03 flashes are composed in order and materialized as explicit CopperRegion shells/holes with provenance. C/O flash polygonization is explicitly evidenced and bounded to 0.005 mm chord error. Disjoint/mixed-axis/invalid cut-ins, LPC with tracks/outlines, Edge.Cuts regions, aperture blocks, and complex macros remain fail-closed |
...
- Gerber `%LPC*%` clear layer polarity uses ordered polygon composition for supported G36/G37 regions plus solid C/R/O D03 flashes. Regions and R flashes are exact; circular/obround boundaries use deterministic inscribed-chord polygonization with explicit `gerber_flash_polygonization` evidence and a 0.005 mm maximum chord-error target. Tracks and outlines remain fail-closed.
