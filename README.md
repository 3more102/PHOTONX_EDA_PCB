...
| Gerber layer polarity | **Partial** | `LPD` is supported; `LPC` composes supported G36/G37 regions, solid C/R/O D03 flashes, and linear circular-aperture D01 tracks in source order. Regions/R flashes are polygon-exact; C/O flash boundaries and track end-caps use deterministic inscribed chords with a 0.005 mm maximum chord-error target. Tessellated arc tracks and outline geometry remain fail-closed |
...
- Gerber `%LPC*%` clear layer polarity uses ordered polygon composition for supported G36/G37 regions, solid C/R/O D03 flashes, and linear circular-aperture D01 tracks. Regions and R flashes are exact; C/O flash boundaries and track end-caps use deterministic inscribed-chord polygonization with explicit `gerber_flash_polygonization` / `gerber_track_polygonization` evidence and a 0.005 mm maximum chord-error target. Tessellated arc tracks and outlines remain fail-closed.
