...
CAPABILITIES=[
    ...
    Capability("Gerber layer polarity","partial","Dark LPD polarity is supported. The internal image engine now preserves ordered dark/clear operations, applies deterministic polygon union/difference composition, and canonicalizes shell/hole output; LPC parsing itself remains fail-closed until parser capture and BoardModel materialization are integrated end-to-end."),
    ...
    Capability("Gerber G74 arcs/regions/macros","partial","Bounded G74 single-quadrant draws are supported when one <=90-degree center candidate is unambiguous. Dark G36/G37 region statements support multiple explicitly closed contours with linear segments plus bounded G74 single-quadrant and G75 multi-quadrant G02/G03 circular boundaries and multiple cut-in holes per contour when every bridge is an opposite fully-coincident linear pair and all bridges in that contour share one axis direction (all horizontal or all vertical). Region holes are explicit geometry and participate in connectivity, DRC, transforms, provenance, and step-repeat. Disjoint fully-coincident cut-in topology, invalid/mixed-axis cut-ins, clear-polarity, Edge.Cuts regions, aperture blocks, and complex macros remain fail-closed."),
