...
CAPABILITIES=[
    ...
    Capability("Gerber G74 arcs/regions/macros","partial","Bounded G74 single-quadrant draws are supported when one <=90-degree center candidate is unambiguous. Dark G36/G37 region statements support multiple explicitly closed contours with linear segments plus G75 G02/G03 circular boundaries, plus one bounded simple cut-in hole per contour when the bridge is a fully-coincident opposite horizontal/vertical linear pair. Region holes are represented explicitly and participate in geometry, connectivity, DRC, transforms, provenance, and step-repeat. G74 region arcs, multiple cut-ins per contour, disjoint fully-coincident cut-in topology, clear-polarity, Edge.Cuts regions, aperture blocks, and complex macros remain fail-closed."),
