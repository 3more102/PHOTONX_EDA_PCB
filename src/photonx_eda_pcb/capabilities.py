...
CAPABILITIES=[
    ...
    Capability("Legacy Gerber compatibility","partial","Supports explicit MO or legacy G70/G71 unit declarations, absolute G90/FS-A notation, standalone/modal D01/D02/D03 operations, UTF-8 BOM, and identity legacy transforms; undeclared/conflicting units, G91/FS-I incremental notation, and non-identity AS/IP/MI/OF transforms fail closed with permissive geometry suppression."),
    ...
    Capability("Gerber step-and-repeat","implemented","Supported draws, flashes, outlines, and arcs are expanded deterministically with source provenance; malformed, non-positive, or over-limit repeat states fail closed and suppress permissive file geometry."),
    ...
    Capability("Excellon coordinate mode","partial","Absolute G90/ICI,OFF and incremental G91/ICI,ON coordinates are supported for drill hits and routed endpoints; incremental G85 canned-slot coordinates remain fail-closed until their two-position semantics are modeled."),
    Capability("Excellon G85 slots","partial","Straight canned slots with explicit absolute endpoints are reconstructed; incremental G91/ICI,ON G85 slots remain fail-closed."),
