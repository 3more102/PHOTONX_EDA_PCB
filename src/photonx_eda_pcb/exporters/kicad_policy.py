    ...
    for component in sorted(source_components, key=lambda item: str(item.id)):
        ...
        for row in rows:
            ...
            if footprint_layer == "B.Cu":
                ...
                # when it is placed on B.Cu.  Invert that transform here so
                # the emitted child pad lands back at the recovered board
                # position and orientation after KiCad applies the flip.
