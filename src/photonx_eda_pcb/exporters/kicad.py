...
from .kicad_policy import KICAD_BOARD_FORMAT_VERSION,KICAD_GENERATOR,KICAD_DEFAULT_BOARD_THICKNESS_MM,KICAD_DEFAULT_PAD_TO_MASK_CLEARANCE_MM,declared_copper_layer_names,drill_export_status,kicad_board_layer_specs,kicad_duplicate_object_ids,kicad_net_export_rows,outline_export_status,pad_export_descriptor,pad_export_status,pad_shape_name,slot_geometry,slot_export_status,track_export_status,proven_via_span_export_plan
...
def _drill_lines(board, report, duplicate_object_ids=(), via_export_ids=()):
    lines = []
    duplicate_object_ids=set(duplicate_object_ids)
...
def _via_span_lines(board,net_num,report,exportable,omitted,problems):
    lines=[]
    ...
    for item in exportable:
        ...
        n,_,net_known=_net_binding(
            board,net_num,item["net_id"],drill_id,report
        )
        if not net_known:
            report.skipped_via_spans+=1
            report.skipped_via_span_ids.append(drill_id)
            report.issues.append(KicadExportIssue(
                ...
                drill_id,
                "proven via span net disappeared from the exported net table; via omitted",
            ))
            continue
        x,y=item["at"]
        ...
        lines.append(
            ...
            f'(net {n}) '
            f'(uuid {photonx_uuid("via:"+drill_id)}))'
        )
        report.exported_via_spans+=1
...
def _route_lines(board,net_num,report,duplicate_object_ids=()):
    ...
    for route in sorted(routes,key=lambda item:item.id):
        if route.id in duplicate_object_ids:
            report.skipped_routes+=1
            if route.id not in report.skipped_route_ids:
        ...
        if code=="KICAD_PLATED_ROUTE_PADSTACK_UNPROVEN":
            ...
            message=(
                ...
                "non-plated routes or source-proven plated routed slots can be "
                "represented faithfully by the current KiCad exporter; route omitted"
            )
        report.issues.append(KicadExportIssue(
            "warning",
            code,
