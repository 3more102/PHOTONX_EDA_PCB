...
def export_kicad_with_report(board:BoardModel,path:str|Path)->tuple[Path,KicadExportReport]:
    ...
    lines.extend(_pad_lines(board,net_num,report));lines.extend(_slot_lines(board,net_num,report));_record_region_skips(board,report);_record_route_skips(board,report)
