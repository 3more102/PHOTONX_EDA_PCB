...
def _is_copper_layer(layer):
    if layer in {"F.Cu", "B.Cu"}:
        return True
    if not (isinstance(layer, str) and layer.startswith("In") and layer.endswith(".Cu")):
        return False
    try:
...
def infer_plated_slot_padstack(board, slot, tolerance_mm=.03):
    ...
    if declared_layers == ():
        return PadstackInference(slot.id, None, ("SLOT_X2_COPPER_STACKUP_INVALID",))

    if declared_layers is not None:
        if set(by_layer) != set(declared_layers):
            return PadstackInference(
                slot.id,
                None,
                ("SLOT_X2_COPPER_LAYER_COVERAGE_MISMATCH",),
            )
        ordered_layers = declared_layers
    else:
        ordered_layers = tuple(sorted(by_layer, key=_layer_key))

    selected = [
        sorted(by_layer[layer], key=lambda pad: (pad_shape(pad).area, pad.id))[0]
