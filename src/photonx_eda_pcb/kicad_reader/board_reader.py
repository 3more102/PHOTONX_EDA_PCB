...
def read_kicad_board_text(text):
    ...
    return {"layers":read_layers(root),"nets":read_nets(root),"segments":read_segments(root),"vias":read_vias(root),"footprints":footprints,"edge_lines":read_edge_lines(root),"mechanical_slots":read_mechanical_slots(footprints)}
