...
def evaluate_macro(primitives, variables=None):
    ...
    for key, value in (variables or {}).items():
        ...
        token = str(int(token))
        if token in defined:
            raise ValueError("macro variable $" + token + " is defined more than once")
    ...
    for statement in primitives:
        if isinstance(statement, MacroVariableDefinition):
            ...
            if token in defined:
                raise ValueError("macro variable $" + token + " cannot be redefined")
            environment[token] = eval_expr(substitute(statement.expression, environment))
            defined.add(token)
            continue
