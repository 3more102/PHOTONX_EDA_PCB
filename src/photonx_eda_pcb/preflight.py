...
class InputPreflight:
    ...
    def to_dict(self) -> dict:
        ...
        data["ready_for_strict_reconstruction"] = (
            self.ready_for_strict_reconstruction
        )
        return data
...
def _scan_file(item) -> FilePreflight:
    ...
    try:
        if item.kind == "drill":
            ...
            # Syntax can still be inspected when layer identity is unresolved.
            parsed = GerberRS274XParser(
                item.layer or "F.Cu", strict=False
            ).parse(item.path)
