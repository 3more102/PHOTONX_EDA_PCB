...
def summary(board,report=None):
    ...
    data={"tracks":len(board.tracks),"pads":len(board.pads),"drills":len(board.drills),"slots":len(getattr(board,"slots",())),"regions":len(getattr(board,"regions",())),"outline_segments":len(board.outline),"physical_nets":len(board.nets),"component_hypotheses":len(board.components),"parse_diagnostics":len(board.diagnostics)}
