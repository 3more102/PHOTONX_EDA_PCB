...
def _effects():return "(effects (font (size 1.27 1.27)))"
def write_export_document(doc):
    ...
    for s in sorted(doc.symbols,key=lambda x:(x.reference,x.id)):
        ...
        lines += ["  (symbol",f"    (lib_id {quote(s.library_id)})",f"    (at {number(s.x)} {number(s.y)} {number(s.rotation)})","    (unit 1)","    (in_bom yes)","    (on_board yes)",f"    (uuid {uid})",f"    (property {quote('Reference')} {quote(s.reference)} (at {number(s.x)} {number(s.y-2.54)} 0) {_effects()})",f"    (property {quote('Value')} {quote(s.value)} (at {number(s.x)} {number(s.y+2.54)} 0) {_effects()})",f"    (property {quote('Footprint')} {quote(s.footprint)} (at {number(s.x)} {number(s.y)} 0) {_effects()} hide)","  )"]
    for w in sorted(doc.wires,key=lambda x:x.id):
        pts=" ".join(f"(xy {number(x)} {number(y)})" for x,y in w.points)
        lines += ["  (wire",f"    (pts {pts})","    (stroke (width 0) (type default))",f"    (uuid {stable_uuid('wire',w.id,w.net_id)})","  )"]
