...
class GerberRS274XParser:
    ...
    def _update_attribute_state(
        ...
        if command == "TA":
            # Gerber uses one name-keyed attribute dictionary across domains.
            # Reusing a name changes its domain and replaces the prior entry.
    ...
    def _snapshot_aperture_attributes(self, code: int) -> None:
        """Freeze the current TA state on one successfully defined aperture."""
    ...
    def _begin_region(
        ...
        # Regions snapshot current TA state at G36; they do not inherit
        # aperture attributes from the selected aperture.
