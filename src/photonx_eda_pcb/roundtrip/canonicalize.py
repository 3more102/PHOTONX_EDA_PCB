def canonical_board_dict(board):
    ...
    def slot(s):
        return {
            "id": s.id,
            "start": [round(float(s.start[0]), 6), round(float(s.start[1]), 6)],
            "end": [round(float(s.end[0]), 6), round(float(s.end[1]), 6)],
    ...
    return {
            ...
            [
                {
                    "id": x.id,
                    "center": pt(x.center),
                    "size": [round(x.size_x, 6), round(x.size_y, 6)],
                    "shape": x.shape,
                    "layer": x.layer,
                    "drill": x.drill,
                    "net_id": x.net_id,
        ...
        "outline": sorted(
            [
                {"id": x.id, "start": pt(x.start), "end": pt(x.end)}
