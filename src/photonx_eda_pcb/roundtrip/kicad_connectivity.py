...
from ..exporters.kicad_policy import KICAD_BOARD_FORMAT_VERSION, KICAD_GENERATOR, KICAD_DEFAULT_BOARD_THICKNESS_MM, KICAD_DEFAULT_PAD_TO_MASK_CLEARANCE_MM, declared_copper_layer_names, drill_export_status, kicad_board_layer_rows, kicad_duplicate_object_ids, kicad_net_export_rows, outline_export_status, pad_export_descriptor, pad_export_status, proven_via_span_export_plan, slot_export_status, track_export_status
...
def _observed_copper_overrides(readback):
    ...
    for footprint in readback.get("footprints", ()):
        if footprint.get("name") not in _PHOTONX_FOOTPRINT_NAMES:
            continue
        fp_overrides = _active_overrides(
            dict(footprint.get("copper_overrides", {}))
        )
        if fp_overrides:
            out.append(
                {
...
def compare_kicad_connectivity(board, readback, export_report=None):
    ...
    recovered slots, and exact non-plated routed paths exported as NPTH route footprints. Proven plated via spans are exported and round-trip checked only when their annular geometry, complete layer support, and reconstructed net are exactly representable; all other proven spans remain explicit source losses. Deterministic PhotonX UUIDs are part of the supported
