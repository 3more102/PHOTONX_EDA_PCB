...
def reconstruct(directory: str | Path, config: ReconstructionConfig | None = None) -> ReconstructionResult:
    ...
    board = BoardModel(metadata={"source_directory": str(Path(directory))})
    files = discover_manufacturing_files(directory)
    if not files:
        raise FileNotFoundError(f"no manufacturing files found in {directory}")
    for item in files:
        if item.kind == "drill":
        ...
        layer = item.layer
        if layer is None:
            board.diagnostics.append(ParseDiagnostic("warning", "UNKNOWN_LAYER", "Gerber file skipped because layer could not be inferred", str(item.path)))
            continue
