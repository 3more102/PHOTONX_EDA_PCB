...
def native_point_radius_candidates(index, queries):
    library = _load_library()
    ids = tuple(index.ids())
    query_values = tuple((float(x), float(y), float(radius)) for x, y, radius in queries)

    if len(ids) > 0xFFFFFFFF or len(query_values) > 0xFFFFFFFF:
        raise NativeBackendUnsupported(
            "native backend supports at most 2^32-1 boxes/queries"
        )
    ...
    query_array_type = _NativePointQuery * len(query_values)
    native_queries = query_array_type(
        *(_NativePointQuery(x, y, radius) for x, y, radius in query_values)
    )
    ...
    status = int(
        library.photonx_point_radius_candidates(
            ...
            ctypes.c_uint32(len(ids)),
            native_queries,
            ctypes.c_uint32(len(query_values)),
            ctypes.c_double(cell_size),
            None,
            ctypes.c_uint32(0),
            ctypes.byref(required),
    ...
    if written.value != required.value:
        raise NativeBackendUnavailable(
            "native backend changed point-query match count between sizing and fill calls"
        )

    grouped = [[] for _ in query_values]
    for i in range(written.value):
        query_index = int(out[i].query)
        point_index = int(out[i].point)
        if query_index >= len(grouped) or point_index >= len(ids):
            raise NativeBackendUnavailable("native backend returned an invalid match index")
        grouped[query_index].append(ids[point_index])
    return grouped
