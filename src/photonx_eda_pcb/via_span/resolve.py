...
from .candidates import pads_near_drill
from .evidence import span_evidence

def resolve_via_spans(board,stackup,tolerance_mm=0.15):
    copper=[x.name for x in stackup.copper_layers()]; out=[]
    for d in board.drills:
        pads=pads_near_drill(board,d,tolerance_mm); layers=[p.layer for p in pads if p.layer in copper]
        uniq=[]
        for x in layers:
